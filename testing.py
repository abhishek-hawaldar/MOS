...
['$AMJ000100030001\n', 'GD10PD10\n', 'GD90PD90\n',
 '$DTA\n', 'ABC\n', '$END0001']
    ...
    for q in  range(len(buffer)):
        buffer[q]= buffer[q][:len(buffer[q])-1]
    ...
    #print(buffer)
    b=0
    while (i < len(buffer) and b<20):

        print("starting line check \n\n\n\n\n\n\n",buffer[i])
        b+=1

        if (not buffer[i].find("$AMJ") == -1):
            # it is the start of a program
            print('it is the start of a program')
            print(buffer[i])
            jobcount = buffer[i][9:12]
            # print(jobcount)
            obj.reset()
            print(jobcount)
            i += 1

            # print("mah man:",buffer[i])
            job = ""

            while (buffer[i].find("$DTA") == -1):
                # print(buffer[i])
                job += buffer[i]
                job = job[:len(job)]
                i += 1
        ...
        elif (not buffer[i].find("$DTA") == -1):
            i += 1
            obj.start_execution()

        elif(not buffer[i].find("$END")==-1):
            obj.printmem()
            i+=2
            continue
        #i+=1
